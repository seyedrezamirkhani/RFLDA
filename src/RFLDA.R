...
SaveData <- function(df, file_name, file_format){
  if (tolower(file_format) == "excel") {
    write_xlsx(df, paste(file_name, ".xlsx", sep=""), col_names = FALSE, use_zip64 = TRUE)
...
LoadData <- function(file_name, file_format) {
  if (tolower(file_format) == "excel") {
    df <- read.xlsx(paste(file_name, ".xlsx", sep=""), sheet = 1, colNames = FALSE)
    return(df)
  } else if (tolower(file_format) == "parquet") {
...
{
  ### L:lncRNA(240*1)
  L <- read.xlsx("../input_data/01-lncRNAs-240.xlsx", sheet = 1, colNames = FALSE)
  ### D:diseases(412*1)
  D <- read.xlsx("../input_data/02-diseases-412.xlsx", sheet = 1, colNames = FALSE)
  ### M:miRNA(495*1)
  M <- read.xlsx("../input_data/03-miRNAs-495.xlsx", sheet = 1, colNames = FALSE)
  ### LL:lncRNA-lncRNA functional similarities (240*240)
  LL <- read.xlsx("../input_data/04-lncRNA-lncRNA.xlsx", sheet = 1, colNames = FALSE)
  ### LD:lncRNA-disease associations (240*412)
  LD <- read.xlsx("../input_data/05-lncRNA-disease.xlsx", sheet = 1, colNames = FALSE)
  ### MD:miRNA-disease associations (495*412)
  MD <- read.xlsx("../input_data/06-miRNA-disease.xlsx", sheet = 1, colNames = FALSE)
  ### DD:disease-disease semantic similarities (412*412)
  DD <- read.xlsx("../input_data/07-disease-disease.xlsx", sheet = 1, colNames = FALSE)
  ### LM:lncRNA-miRNA interactions (240*495)
  LM <- read.xlsx("../input_data/08-lncRNA-miRNA.xlsx", sheet = 1, colNames = FALSE)
  ...
  # NOTE! SRM
  # New Code Using sqldf
  #    user  system elapsed 
  # 115.087   0.720 115.751 
  system.time(
    LDExcl0 <- sqldf(c('
...
ComputeFeatureImportance<-function(file_format){
    ...
    rf <- ranger(
      ...
      data = B1,
      mtry = 651,
      importance = 'impurity', # Use 'impurity' for Gini importance or 'permutation' for permutation importance
      num.trees = 500,
      na.action = 'na.omit',
